				...
				<uni-collapse-item title="基础信息" :open="true">
					<view class="gaowaCellBox">
						<view v-for="(item,index) in cellList" :key="item.key">
							...
							<view class="cellItem" v-if="item.key=='key1'">
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			util.request("inspfillin/getBasicInfo_reach", "Get", id, false, false, false, true).then(res => {
				...
				this.cellList[4].value = this.information.cen_long
				this.cellList[5].value = this.information.cen_lat
				this.cellList[6].value = this.information.con_person
				this.cellList[7].value = this.information.con_tel
		...
		methods: {
			...
			preservation() {
				...
				util.request("inspfillin/saveBasicInfo_reach", "POST", formList, false, false, false, true).then(res => {
							...
							util.request("inspfillin/delProlist", "Get", id, false, false, false, true)
								.then(
									res => {
										...
										that.$nextTick(() => {
											that.csListArrl[index].right = '0'

											that.search(); //触发上个页面自定义的go_update方法
										})

										// console.log(that.csListArrl[index])
									})
			...
			addQuestion(number) {
				...
				uni.navigateTo({
					url: '/pages/question/question2?title=' + JSON.stringify(this.tianBaoList) + '&number=' +
						number
				})
			...
			chakan(item) {
				uni.navigateTo({
					url: '/pages/seeandxiu/seeandxiu2?title=' + JSON.stringify(this.tianBaoList) + '&shuju=' +
						JSON
						.stringify(item)
				})
			},
