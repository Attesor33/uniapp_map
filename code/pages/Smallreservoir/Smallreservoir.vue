				...
				<uni-collapse-item title="基础信息" :open="true">
					<view class="gaowaCellBox">
						<view v-for="(item,index) in cellList" :key="item.key">
							...
							<view class="cellItem" v-else-if="item.key=='key1'">
								<label>{{item.label}}</label>
				...
				<uni-collapse-item title="检查情况">
					<view class="gaowaCellBox">
						<view v-for="(item,index) in checkList" :key="item.key">
							<view v-if="['check2'].includes(item.key)">
								<view class="otherCell">{{item.label}}</view>
								<view class="otherCell" style="font-size: 28rpx;">
									<textarea maxlength="500" v-model="item.value" placeholder="请输入"
										:disabled="disabled" />
								</view>
							</view>
	...
	export default {
		...
		methods: {
			...
			preservation() {
				...
				let data = {
					fir_chk_year: this.checkList[0].value,
					gkztpj: this.checkList[2].value,
					is_dc: this.checkList[4].value,
					res_chk_type: this.checkList[3].value,
					ygkpj: this.checkList[1].value,
					obj_id: this.tianBaoList.obj_id,
					plan_id: this.tianBaoList.plan_id,
				}
				util.request("inspfillin/saveChkInfo_res", "Get", data, false, false, false, true).then(res => {
					uni.showToast({
						duration: 1000,
					});
					this.disabled = true
			...
			addQuestion(number) {
				if (number == 1) {
					uni.navigateTo({
						url: '/pages/question/question?title=' + JSON.stringify(this.tianBaoList) + '&number=' +
							number
					})
				} else if (number == 0) {
					uni.navigateTo({
