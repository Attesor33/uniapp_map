<template>
	<view class="box">
		<navBar :title="isEdit?title:title1">
			<template #left>
		...
		<view class="bottomBtn" @click="add">保存</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			this.shuju = JSON.parse(option.shuju)
			this.weiyi = JSON.parse(option.title)
			console.log(this.shuju)
			this.cellList[0].value = this.shuju.chk_type
			this.cellList[1].value = this.shuju.chk_pro
			this.cellList[2].value = this.shuju.pro_des
			this.cellList[3].value = this.shuju.pro_lv
			this.cellList[4].value = this.shuju.rev_des
			this.cellList[5].value = this.shuju.group_nm
			let date = this.cellList[6].value
			var dates = new Date(date).toJSON();
			this.cellList[6].value = new Date(+new Date(dates) + 8 * 3600 * 1000)
				.toISOString()
				.replace(/T/g, " ")
				.replace(/\.[\d]{3}Z/, "");
			this.imgList = this.shuju.filelist
			this.search()
		},
		methods: {
			back() {
			...
			add() {
				this.shuju.chk_type = this.cellList[0].value,
					this.shuju.chk_pro = this.cellList[1].value,
					this.shuju.pro_des = this.cellList[2].value,
					this.shuju.pro_lv = this.cellList[3].value,
					this.shuju.rev_des = this.cellList[4].value,
					this.shuju.filelist = this.imgList,

					console.log(this.shuju)
					let pages = getCurrentPages(); //获取小程序页面栈
					let beforePage = pages[pages.length - 2]; //获取上个页面的实例对象
				util.request("inspfillin/upProlist", "POST", this.shuju, false, false, false, true).then(res => {
					uni.showToast({
						duration: 1000,
					});
					beforePage.search(); //触发上个页面自定义的go_update方法
